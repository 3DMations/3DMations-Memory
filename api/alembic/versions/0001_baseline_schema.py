"""baseline schema — mirrors db/init.sql.bak (pre-Batch-C).

Creates the entries table, upsert_entry() function, and indexes as they
existed before the client_type column was added.
"""
...
def upgrade() -> None:
    ...
    # AUDIT-003: upsert_entry uses now() for updated_at, never EXCLUDED.updated_at.
    op.execute(
        """
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_machine    ON entries(machine_id);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_category   ON entries(category);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_tags       ON entries USING gin(tags);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_status     ON entries(status);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_recurrence ON entries(recurrence_count DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_confidence ON entries(confidence_score DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS idx_entries_last_seen  ON entries(last_seen DESC);")
...
def downgrade() -> None:
    # Baseline is never rolled back — dropping entries would destroy all memory.
    # Downgrade is intentionally a no-op. See CLAUDE.md destructive_action_guard.
    pass
